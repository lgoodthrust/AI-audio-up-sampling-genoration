...
class PairedAudioDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        low_audio = low_audio / (np.max(np.abs(low_audio)) + 1e-8)
        high_audio = high_audio / (np.max(np.abs(high_audio)) + 1e-8)
...
def model_ctrl(b_size=32, t_frac=0.8, lr=1e-5, epoch_steps=32, samp_rate=44100, dk_rate=1e-25, eps_rate=1e-8):
    # Data Loader
